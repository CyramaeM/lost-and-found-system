...
def Search():
    ...
    if search_text.get() != "":
        ...
        try:
            ...
            cursor.execute("SELECT lost_items.item_id, lost_items.report_date,lost_items.claim_date, lost_items.description, reporter.reporter_id, claimant.claimant_id FROM lost_items LEFT JOIN claimant ON lost_items.claimant_id = claimant.claimant_id LEFT JOIN reporter ON lost_items.reporter_id = reporter.reporter_id WHERE report_date = %s", (formatted_date,))
            ...
            for data in fetch:
                ...
                tree.insert("", "end", values=(item_id, report_date,claim_date, description, reporter_id, claimant_id, status))
            cursor.close()
            db.close()
        except ValueError:
...
def Displayform():
    ...
    def register():
        ...
        global entry_first_name,entry_last_name,entry_contact_number,entry_gender,entry_date,entry_description
        global droplist
        ...
        def Report():
                ...
                try:
                    ...
                    sql1 = ("INSERT INTO lost_items (reporter_id, role, report_date, description) VALUES (%s, %s, %s, %s)")
                    val2 = (reporter_id, "reporter", formatted_date, describe)
        ...
        if not tree.selection():
            ...
            if result == 'yes':
                ...
                def update1():
                    ...
                    cursor.execute(
                        "UPDATE lost_items SET claimant_id=%s, role=%s, claim_date=%s WHERE item_id=%s",
                        (claimant_id, "claimant", formatted_date, item_id)
                    )
    ...
    vsb.place(x=850,y=170,height=225)
    ...
    tree.heading("3",text="Claim date")
    tree.heading("4",text="Description")
    tree.heading("5",text="Reporter ID")
    tree.heading("6",text="Claimant ID")
